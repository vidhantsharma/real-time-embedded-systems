...
#define   PRESCALER_DIV16   5                       // 1 MHz clock just oldname
...
void servo_out(int traj, int dirn, int speed ){
    ...
    if(dirn == 0){
        ...
        if(traj == 1){
            ...
            // seqB = mid_counter; seqA = mid_counter;
            seqB = countertop; seqA = countertop; //Stop   //Stop
        }
    }
    // printf("\n[servo_out]seqA: %d, seqB: %d", seqA, seqB);
